...
class TestParseGuiSteps(unittest.TestCase):
    ...
    def test_screenshot_region(self):
        steps = m.parse_gui_steps("screenshot --region 100,200 800x600 panel")
        self.assertEqual(steps[0]["action"], "screenshot")
        self.assertEqual(steps[0]["region"], (100, 200, 800, 600))
        self.assertEqual(steps[0]["name"], "panel")

    def test_screenshot_region_no_name(self):
        steps = m.parse_gui_steps("screenshot --region 0,0 1920x1080")
        self.assertEqual(steps[0]["region"], (0, 0, 1920, 1080))
        self.assertIsNone(steps[0]["name"])

    def test_screenshot_window(self):
        steps = m.parse_gui_steps('screenshot --window Firefox browser')
        self.assertEqual(steps[0]["action"], "window-screenshot")
